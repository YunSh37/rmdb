...
## 项目定位
C++17 关系型数据库，实现：查询执行、唯一索引、聚合与分组、半连接、事务、MVCC、故障恢复（含检查点）。测试点详见 `测试说明文档v3.0.md`，一致性规则见 `数据一致性检验规则.pdf` 和 `一致性检测2.pdf`。
...
- pdf文件读取如有困难，先用其它工具转换（如找到合适方法请同步更新此处处理）

### 2. Git 管理（git路径：D:\360downloads\git\Git\cmd\git.exe）
...
```
对于每一个题目完成后告知用户相应的测试方法步骤，便于用户亲自测试。
